//Description: This class is used when the player wants to equip 
//something. It has two constructors, the second one
// is used when the player wants to equip a trinket or 
//when the player has a character that can
// duel wield and the game wants to know in what slot 
//the equipping weapon should be placed.
using UnityEngine;
...
public class EquipItem 
{
    ...
    private EquipItem() { }
    public EquipItem(
        Item item, GameObject go, EquipmentManager equipmentManager)
    {
        Debug.Log("EquipItem RUNNING | EquipItem RUNNING |" + 
            "EquipItem RUNNING | EquipItem RUNNING |");
        ...
        Equip(go, item);
        Debug.Log("EquipItem DONE | EquipItem DONE" + 
            "| EquipItem DONE | EquipItem DONE | EquipItem DONE | ");

    }
    public EquipItem(
        int equipmentSlot, Item item, EquipmentManager equipmentManager)
    {
        ...
        Item currentlyEquippedItem = 
            equipmentManager.EquipmentSlots[equipmentSlot].
            GetComponent<InventorySlot>()._Item;

        if (currentlyEquippedItem != null)
        {
            new MoveItemBetweenInventoryAndEquipmentSlot(
                currentlyEquippedItem, slot, playerInventory);
        }
            new MoveItemBetweenInventoryAndEquipmentSlot(
                item, playerInventory, slot);
