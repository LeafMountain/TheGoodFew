...
public class UnitSetupController : CombatElement {
	...
	private void FindTurnOrderObjects() {
		...
		for (int i = 0; i < turnOrderObjects.Length; i++){
			App.Model.Turn.AddUnit(new UnitModel(null, turnOrderObjects[i], App.Controller.Board.GetCell(new Vector2(turnOrderObjects[i].transform.position.x, turnOrderObjects[i].transform.position.z))));
		}
	}

	private void PlaceUnits(){
		for (int i = 0; i < App.Model.Turn.GetTurnOrder().Count; i++)
		{
			UnitModel unit = App.Model.Turn.GetTurnOrder()[i];
